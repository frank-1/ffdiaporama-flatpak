...
void wgt_QCompositionWidget::RefreshBackgroundImage() {
    ...
    for (int i=0;i<CompositionList->List.count();i++) {
        // Draw composition
        CompositionList->List[i].DrawCompositionObject(&P,0,0,xmax,ymax,true,0,NULL,1,NULL,false);
        // Draw border
...
void wgt_QCompositionWidget::MakeFormIcon(QComboBox *UICB) {
    for (int i=0;i<UICB->count();i++) {
        ...
        ADJUST_RATIO=1;
        Object.DrawCompositionObject(&Painter,0,0,32,32,true,0,NULL,1,NULL,false);
        Painter.end();

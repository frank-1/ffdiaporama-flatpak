...
void wgt_QImageFilterTransform::SetFilter(cFilterTransformObject *TheFilter,cDiaporamaObject *TheDiaporamaObject) {
    ...
    if (Filter!=NULL) {
        ...
        if (DiaporamaObject->Video!=NULL)        SourceImage=DiaporamaObject->Video->ImageAt(true,DiaporamaObject->Parent->ApplicationConfig->PreviewMaxHeight,0,true,true,NULL,1);

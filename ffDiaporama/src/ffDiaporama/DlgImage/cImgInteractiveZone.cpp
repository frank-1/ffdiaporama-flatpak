...
void cImgInteractiveZone::paintEvent(QPaintEvent *) {
    ...
    CurImgSelRect=PolygonToRectF(ComputePolygon(BackgroundForm,TmpImgSelRect.left(),TmpImgSelRect.top(),TmpImgSelRect.width(),TmpImgSelRect.height(),TmpImgSelRect.width()/2+TmpImgSelRect.left(),TmpImgSelRect.height()/2+TmpImgSelRect.top()));
    CurScrSelRect=PolygonToRectF(ComputePolygon(BackgroundForm,TmpScrSelRect.left(),TmpScrSelRect.top(),TmpScrSelRect.width(),TmpScrSelRect.height(),TmpScrSelRect.width()/2+TmpScrSelRect.left(),TmpScrSelRect.height()/2+TmpScrSelRect.top()));
...
void cImgInteractiveZone::ComputeNewCurSelRect(QRectF &ScrRect,QRectF &ImgRect) {
    ...
    ScrRect       =PolygonToRectF(ComputePolygon(BackgroundForm,TmpRect.left(),TmpRect.top(),TmpRect.width(),TmpRect.height(),TmpRect.center().x(),TmpRect.center().y()));
    ...
    ImgRect       =PolygonToRectF(ComputePolygon(BackgroundForm,TmpRect.left(),TmpRect.top(),TmpRect.width(),TmpRect.height(),TmpRect.center().x(),TmpRect.center().y()));
}
...
void cImgInteractiveZone::mousePressEvent(QMouseEvent *event) {
    ...
    if ((event->button()==Qt::LeftButton)&&(event->modifiers()==Qt::NoModifier)) {
        ...
        CurImgSelRect=PolygonToRectF(ComputePolygon(BackgroundForm,CurImgSelRect.left(),CurImgSelRect.top(),CurImgSelRect.width(),CurImgSelRect.height(),CurImgSelRect.width()/2+CurImgSelRect.left(),CurImgSelRect.height()/2+CurImgSelRect.top()));
        CurScrSelRect=PolygonToRectF(ComputePolygon(BackgroundForm,CurScrSelRect.left(),CurScrSelRect.top(),CurScrSelRect.width(),CurScrSelRect.height(),CurScrSelRect.width()/2+CurScrSelRect.left(),CurScrSelRect.height()/2+CurScrSelRect.top()));
        TransfoType  =NOTYETDEFINED;
        Move_X       =0;
        Scale_X      =0;
        Move_Y       =0;
